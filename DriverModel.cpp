...
void logg(int, int, int, int, int, int, int, double);
void logg1(double);
double link_signal12;
double link_signal23;
static boolean is_file_deleted = false;
...
void logg(int current_veh, double current_time, double S, double gross_distance, double v_delta, double distance, double current_velocity, double desired_velocity,
	double car_following_acceleration, double current_acceleration, double desired_acceleration)
{
	if (!is_file_deleted)
	{
		std::remove("C:\\Users\\yunpe\\Documents\\logg.txt");
		is_file_deleted = true;
	}
	FILE*fp;
	fopen_s(&fp, "C:\\Users\\yunpe\\Documents\\logg.txt", "a");
	fprintf(fp, "%d,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf\n", current_veh, current_time, S, gross_distance, v_delta, distance, current_velocity, desired_velocity, car_following_acceleration, current_acceleration, desired_acceleration);
	fflush(fp);
	fclose(fp); //for logging the data
}
